...
class BacktestTrade(Base):
    ...
    signal_message_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("channel_messages.id", ondelete="SET NULL"), nullable=True
    )
    close_message_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("channel_messages.id", ondelete="SET NULL"), nullable=True
    )

    # Technical enrichment at entry
    entry_rsi: Mapped[float | None] = mapped_column(Float, nullable=True)
    entry_macd: Mapped[float | None] = mapped_column(Float, nullable=True)
    entry_bollinger_position: Mapped[float | None] = mapped_column(Float, nullable=True)  # -1 to 1
    entry_vwap_distance: Mapped[float | None] = mapped_column(Float, nullable=True)
    entry_sma_20_distance: Mapped[float | None] = mapped_column(Float, nullable=True)
    entry_sma_50_distance: Mapped[float | None] = mapped_column(Float, nullable=True)
    entry_volume_ratio: Mapped[float | None] = mapped_column(Float, nullable=True)  # vs 20-day avg
    entry_atr: Mapped[float | None] = mapped_column(Float, nullable=True)

    # Market context
    market_vix: Mapped[float | None] = mapped_column(Float, nullable=True)
    market_spy_change: Mapped[float | None] = mapped_column(Float, nullable=True)
    ...
    option_flow_sentiment: Mapped[str | None] = mapped_column(String(20), nullable=True)  # bullish/bearish/neutral
    gex_level: Mapped[float | None] = mapped_column(Float, nullable=True)
